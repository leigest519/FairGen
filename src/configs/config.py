...
class NetworkConfig(BaseModel):
    rank: int = 1
    alpha: float = 1.0
    # scaling factor at inference for adapter strength
...
class TrainConfig(BaseModel):    
    ...
    # FairGen: orthogonal regularization
    orthogonal_regularization: bool = False
    orthogonal_weight: float = 0.0
    # paths to previously trained adapters to enforce orthogonality against (safetensors)
    orthogonal_reference_paths: list[str] = None
